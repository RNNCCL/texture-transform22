...
program
  ...
  .option(
    '--swizzle <string>',
    'Convert channels using glsl swizzle style specification',
    {
  ...
  .action(async ({ args, options, logger }) => {
    ...
    logger.info(
      `Converting ${input} to ${output} with quality ${quality} and swizzle ${swizzle}`
    );
...
program
  ...
  .option(
    '--channels <swizzles>',
    'A series of swizzles, one for each input image that specific which channels to extract and combine.',
    {
      validator: program.STRING,
      default: 'auto'
    }
  ...
  .action(async ({ args, options, logger }) => {
    ...
    logger.info(
      `Packing ${inputs} to ${output} with quality ${quality} and channels ${channels}`
    );
...
program
  ...
  .help(
    'Many common image modifications can be expressed as a function applied to each pixel in the image'
  )
  .argument('<input>', INPUT_IMAGE)
  .argument('<output>', OUTPUT_IMAGE)
  .option('--quality <integer>', 'The quality of the result in percent', {
  ...
  .option(
    '--from <role>',
    'The role of the texture: bump, normal, reflect, gloss, metallic, rough, f0, ior.',
    {
      validator: program.STRING,
      required: true
    }
  ...
  .action(async ({ args, options, logger }) => {
    ...
    logger.info(
      `Converting ${input} (${from}) to ${output} (${to}) with quality ${quality}`
    );
