import { Color3, color3Lerp, color3MultiplyByScalar, color3Subtract } from "@threeify/math";
import { Texture, createTexture, getTexturePixel, setTexturePixel, textureIterator } from "../../Texture";
import { assert } from "../../helpers/assert";
import { color3LinearToLuma, color3MaxComponent } from "../../helpers/Color3.Helpers";
...
function solveMetallic(F0: Color3, diffuse: number, specular: number, oneMinusSpecularStrength: number) {
    if (specular < F0.r) {
        return 0;
    }

    var a = F0.r;
    var b = diffuse * oneMinusSpecularStrength / (1 - F0.r) + specular - 2 * F0.r;
    var c = F0.r - specular;
...
export function pbrToSpecularGlossiness(
    ...
    metallicTexture= createTexture( diffuseTexture.width, diffuseTexture.height, 3),
    roughnessTexture= createTexture( diffuseTexture.width, diffuseTexture.height, 1)
): { baseTexture: Texture, metallicTexture: Texture, roughnessTexture: Texture } {
    ...
    const textures = { specularTexture, glossinessTexture, baseTexture, metallicTexture, roughnessTexture,  };
    ...
    textureIterator(diffuseTexture, (x, y) => {
        ...
        var glossiness = glossinessPixel[0];

        var oneMinusSpecularStrength = 1 - color3MaxComponent( specularColor );
        var metallic = solveMetallic(
            dielectricSpecular,
            color3LinearToLuma( diffuseColor ),
            color3LinearToLuma( specularColor ), oneMinusSpecularStrength);
        
        color3MultiplyByScalar( diffuseColor, oneMinusSpecularStrength / (1 - dielectricSpecular.r) / Math.max(1 - metallic, epsilon ), baseColorFromDiffuse);

        color3Subtract( specularColor, color3MultiplyByScalar( dielectricSpecular, 1 - metallic, tempColor ), baseColorFromSpecular );
        color3MultiplyByScalar( baseColorFromSpecular, 1 / Math.max(metallic, epsilon), baseColorFromSpecular );

        color3Lerp( baseColorFromDiffuse, baseColorFromSpecular, metallic * metallic, baseColor );
